...
static void wiper(Device* device) {
    ...
    const char* titleheaders[] = { "Wiper", NULL };
    //const char* items[] = { " No", " Yes", NULL };
    const char* items[fs_table->num_entries];
    const char* fitems[fs_table->num_entries];
    items[fs_table->num_entries]=NULL;
    for (int i = 0; i < fs_table->num_entries ; i++){
      const Volume* v = &fs_table->recs[i];
      char *mountpoint = v->mount_point+1;
      if (strcmp(v->fs_type, "ext4") == 0){
        items[i]=mountpoint;
        fitems[i]=v->mount_point;
      } else {
        items[i]="UNSUPPORTED";
        fitems[i]=NULL;
      }
    ...
    int chosen_item = get_menu_selection(titleheaders, items, true, 0, device);
    if (fitems[chosen_item]==NULL) return;
    const char* wipeheaders[] = {"Wipe this partition:", items[chosen_item], NULL};
    const char* wipeitems[] = {" No ", " Yes ", NULL};
    int wipeorno = get_menu_selection(wipeheaders, wipeitems, true, 0, device);
    if (wipeorno == 0)return ;
    erase_volume(fitems[chosen_item]);
}
